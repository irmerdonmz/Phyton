...
def transform(legacy_data: Dict[int, List[str]]) -> Dict[str, int]:
    ...
    for key, value in legacy_data.items():
        x = dict.fromkeys(value, key)
        dict1.update(x)
        print(dict1)
    sortedDict = (sorted(dict1.items(), key=lambda x: x[0]))
    lowerDict = {k.lower(): v for k,v in sortedDict}
    print(dict(lowerDict))
    return(dict(lowerDict))
